...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!isUuid(userId)) return json({ error: 'Invalid userId (must be UUID)' }, 400);
    ...
    const { data: children, error: childErr } = await admin
      ...
      .select('id')
      .eq('parent_id', userId);
    if (childErr) return json({ error: 'Failed to load children', details: childErr.message }, 500);
    ...
    return json({ success: true, deleted_children: childIds.length, partial_errors: errors });
  } catch (e) {
