...
// ExerciseWriteDigit is the odd one out among the four canvas-handwriting
// screens: unlike ExerciseWriteNumber/ExerciseNumberLine, it does NOT call a
// Supabase edge function at all. Recognition is fully client-side — it
// rasterizes the target digit's SVG path onto an offscreen canvas and
// compares pixel-by-pixel overlap against what the child drew (see
// checkDrawing() in the component). So there's no network/edge-function
// failure path to test here; instead we make the two getImageData() calls
// inside checkDrawing deterministic (see queueRecognition below) to force the
// correct/incorrect branches without needing real canvas rasterization, which
// jsdom doesn't support anyway.
//
...
// ResizeObserver, both of which this component uses, so both are polyfilled
// below as inert no-ops.
//
...
vi.mock('@/hooks/useExerciseId', () => ({ useExerciseId: () => 'ex-write-digit' }));
...
describe('ExerciseWriteDigit', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.spyOn(Math, 'random').mockReturnValue(0); // deterministic target digit: floor(0*10) = 0
  ...
  it('cancels the pending demo-animation frame on unmount', () => {
    ...
    const { unmount } = render(<ExerciseWriteDigit />);
  ...
  it('on a correct recognition: celebrates, advances the iteration, and resets for the next round', async () => {
    ...
    expect(screen.getByText('Super! Minder hulp nu!')).toBeInTheDocument();
